...
(async () => {
  ...
  let receivedNew = false,
    receivedMention = false,
    gotAck = false;
  const received = {};
  sockB.on("message:new", (m) => {
    received.new = m;
    receivedNew = true;
    console.log("B got message:new:", m.type, JSON.stringify(m.text));
  });
  sockB.on("mention:new", (m) => {
    received.mention = m;
    receivedMention = true;
    console.log("B got mention:new for @", m.username);
  });
  sockA.on("message:ack", (m) => {
    gotAck = true;
    console.log("A got ack:", m.id);
  });
  ...
  console.log(
    "sticker delivered:",
    received.new && received.new.type === "text"
      ? "(text first, sticker next expected)"
      : received.new?.type,
  );
  console.log(
    "B received text matches:",
    received.new &&
      received.new.text &&
