...
async function post(p, b, t) {
  ...
  if (!r.ok) throw new Error(p + " " + r.status + " " + x);
  return JSON.parse(x);
}
...
(async () => {
  ...
  const c = await post("/auth/register", {
    username: "gc_" + s,
    fullName: "GC",
    email: "gc_" + s + "@x.com",
    password: "password123",
  ...
  const conv = await post(
    "/conversations/group",
    { title: "Team " + s, memberIds: [bUser.id, cUser.id] },
    a.accessToken,
  );
  console.log(
    "group created:",
  ...
  await post(
    "/devices/register",
    { token: "FAKE_FCM_TOKEN_B_" + s, platform: "android" },
    b.accessToken,
  );
  await post(
    "/devices/register",
    { token: "FAKE_FCM_TOKEN_C_" + s, platform: "ios" },
    c.accessToken,
  );
